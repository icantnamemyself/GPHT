...
class Dataset_Pretrain(Dataset):
    def __init__(self, root_path, flag='train', size=None,
        ...
        self.bounds = [0]
        self.num_channels = []
        for file in ['ETTh1', 'ETTh2', 'ETTm1', 'ETTm2']:
            Data = data_dict[file]('dataset/ETT-small/', flag, size, features, file + '.csv', target, scale, timeenc,
                                   freq, percent=percent)
            self.num_channels.append(Data.data_x.shape[-1])
            self.data_xs.append(Data.data_x)
            self.data_ys.append(Data.data_y)
    ...
    def __getitem__(self, index):
        for i in range(len(self.num_channels)):
            if self.bounds[i] <= index < self.bounds[i + 1]:
                break
        index -= self.bounds[i]
        channel = index % self.num_channels[i]
        s_begin = index // self.num_channels[i]
